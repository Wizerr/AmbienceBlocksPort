...
import com.wizerr.ambienceblocks.ambience.conds.*;
...
public class CondsUtil {
    //nbt stuff
    public static CompoundTag toNBT(AbstractCond cond) {
        CompoundTag compound = cond.toNBT();
        compound.putString("condName", cond.getName()); //put type information
        return compound;
    }

    public static AbstractCond fromNBT(CompoundTag nbt) {
    ...
    public static void toBuff(AbstractCond cond, FriendlyByteBuf buf) {
        buf.writeInt(CondList.getIntFromCond(cond));
        cond.toBuff(buf);
    }

    public static AbstractCond fromBuff(FriendlyByteBuf buf) {
        int id = buf.readInt();
