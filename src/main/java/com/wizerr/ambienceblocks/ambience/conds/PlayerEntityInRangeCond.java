...
public class PlayerEntityInRangeCond extends AbstractCond {
    ...
    public boolean isTrue(Player player, Level worldIn, IAmbienceSource sourceIn) {
        ...
        // Crée une AABB autour du joueur
        AABB box = AABB.unitCubeFromLowerCorner(getPlayerPos(player)).inflate(searchRange);

        List<Entity> entities = worldIn.getEntities(null, box);
        for (Entity entity : entities) {
            if (entity != player) {
                ResourceLocation key = BuiltInRegistries.ENTITY_TYPE.getKey(entity.getType());
                if (stringValidation(key.toString(), this.entity)) {
