...
public class PlayerPosWithinRegionCond extends AbstractCond {
    ...
    public void fromNBT(CompoundTag nbt) {
        Vec3 startPos = NBTHelper.readVec3d(nbt.getCompound(START));
        Vec3 finalPos = NBTHelper.readVec3d(nbt.getCompound(FINAL));
        xS = startPos.x;
        yS = startPos.y;
        zS = startPos.z;
        xF = finalPos.x;
    ...
    private void updateBoundingBox() {
        boundingBox = new AABB(getStartPos(), getFinalPos());
    }
    ...
    private Vec3 getFinalPos() {
        return new Vec3(xF, yF, zF);
    }
}
