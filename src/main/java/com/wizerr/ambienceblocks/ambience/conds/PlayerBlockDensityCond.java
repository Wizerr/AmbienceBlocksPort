...
import com.wizerr.ambienceblocks.ambience.util.messenger.AmbienceWidgetLabel;
...
public class PlayerBlockDensityCond extends AbstractCond {
    ...
    private int countBlocksWithinAABB(AABB bb, Level world) {
        ...
        for (int x = i; x < j; ++x) {
            for (int y = k; y < l; ++y) {
                for (int z = i1; z < j1; ++z) {
                    BlockState blockstate = world.getBlockState(pos.set(x, y, z));
                    ResourceLocation key = BuiltInRegistries.BLOCK.getKey(blockstate.getBlock());

                    if (stringValidation(key.toString(), block)) {
                        counter++;
                    }
                }
            }
